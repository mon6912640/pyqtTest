...
def compress_online(source_path, output_path):
    ...
    # if proxy:
    #     tinify.proxy = proxy
    show_log('上传压缩图片...' + source_path, end=' ')
    result = False
    try:
        ...
        # 如果key值无效 换一个key继续压缩
        show_log("key值无效 换一个继续。。。")
        try:
            online_key = next(online_key_list_iter)
        except:
            show_log('配置的key已经用完了，请到官网申请更多的key https://tinypng.com/developers')
            result = False
        else:
            result = compress_online(source_path, output_path)  # 递归方法 继续读取
    except errors.ClientError as e:
        # Check your source image and request options.
        show_log("Check your source image and request options. %s" % e.message)
        result = False
    except errors.ServerError as e:
        # Temporary issue with the Tinify API.
        show_log("Temporary issue with the Tinify API. %s" % e.message)
        result = False
    except errors.ConnectError as e:
        # A network connection error occurred.
        show_log("网络故障。。。休息1秒继续")
        time.sleep(1)
        result = compress_online(source_path, output_path)  # 递归方法 继续读取
    except Exception as e:
        # Something else went wrong, unrelated to the Tinify API.
        show_log("Something else went wrong, unrelated to the Tinify API.  %s" % e)
        result = False
    else:
...
def handle_file(source_path, target_path):
    ...
    if values is None:  # 数据库不存在，则需要上传压缩图片
        ...
        if cr.fetchone() is not None:  # 主键不存在但存在于压缩值中，则表示这个图片是已经压缩过了，无需做处理
            ...
            show_log(str(source_path) + ' 已经压缩过了，不做处理')
            pass
        else:
            need_upload = True
    else:  # 数据库存在
        ...
        if os.path.exists(cache_file):  # 存在缓存文件，则从缓存文件夹复制出来
            ...
            show_log(str(source_path) + ' 从缓存取出')
            pass
        else:  # 不存在缓存文件，则上传压缩图片
            need_upload = True
            show_log(str(source_path) + ' 数据库存在，但没有缓存')
            pass

    if need_upload:
        if not compress_online(str(source_path), str(target_path)):
            show_log("压缩失败，检查报错信息")
            close_db()
            return False
        else:
            # 上传压缩成功并下载压缩文件回来了
...
def run(p_source_path, p_output_path):
    ...
    show_log('输出 %s 个文件' % file_count)
    show_log('总用时{0}'.format(end - start))
